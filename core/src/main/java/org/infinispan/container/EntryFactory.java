...
/**
 * A factory for constructing {@link org.infinispan.container.entries.ContextEntry} instances for use in the {@link org.infinispan.context.InvocationContext}.
 * Implementations of this interface would typically wrap an internal {@link org.infinispan.container.entries.CacheEntry}
 * with an {@link org.infinispan.container.entries.ContextEntry}, optionally acquiring the necessary locks via the
...
public interface EntryFactory {
    ...
    * levels and the presence of an ongoing JTA transaction may force this to be a proper, wrapped ContextEntry.  The entry
    ...
    */
   ContextEntry wrapEntryForReading(InvocationContext ctx, Object key) throws InterruptedException;
    ...
    * @throws InterruptedException when things go wrong, usually trying to acquire a lock
    */
   ContextEntry wrapPreviouslyReadEntry(InvocationContext ctx, CacheEntry entry) throws InterruptedException;

   /**
    * Used for wrapping individual keys when clearing the cache. The wrapped entry is added to the
    * supplied InvocationContext.
    */
   ContextEntry wrapEntryForClear(InvocationContext ctx, Object key) throws InterruptedException;

   /**
    * Used for wrapping a cache entry for replacement. The wrapped entry is added to the
    * supplied InvocationContext.
    */
   ContextEntry wrapEntryForReplace(InvocationContext ctx, ReplaceCommand cmd) throws InterruptedException;
    ...
    */
   ContextEntry wrapEntryForRemove(InvocationContext ctx, Object key, boolean skipRead) throws InterruptedException;
    ...
    */
   ContextEntry wrapEntryForDelta(InvocationContext ctx, Object deltaKey, Delta delta) throws InterruptedException;
    ...
    */
   ContextEntry wrapEntryForPut(InvocationContext ctx, Object key, CacheEntry ice,
