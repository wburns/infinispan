...
public class PublisherRequestCommand<K> extends BaseRpcCommand implements TopologyAffectedCommand {
   ...
   public CompletableFuture<Object> invokeAsync() throws Throwable {
      if (entryStream) {
         return (CompletableFuture<Object>) lpm.entryPublisherOperation(parallelStream, segments, keys, excludedKeys,
               includeLoader, deliveryGuarantee, transformer, finalizer);
      } else {
         return (CompletableFuture<Object>) lpm.keyPublisherOperation(parallelStream, segments, keys, excludedKeys,
               includeLoader, deliveryGuarantee, transformer, finalizer);
      }
   }
   ...
   public boolean canBlock() {
      ...
      // provides the transformation, so it could block
