...
abstract class BaseQueueingSegmentListener<K, V, E extends Event<K, V>> implements QueueingSegmentListener<K, V, E> {
   ...
   @Override
   public boolean test(SegmentCompletionPublisher.Notification<CacheEntry<K, V>> cacheEntryNotification) throws Throwable {
      if (cacheEntryNotification.isValue()) {
         ...
         return value != QueueingSegmentListener.REMOVED;
      }
