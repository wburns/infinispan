...
public class BPlusTree<V> {
    ...
    */
   public V get(byte[] key) throws IOException {
      Node<V> node = root;
   ...
   /**
    * Thrown by a {@link PublishFunction} or {@link SoftBPlusTree.NodeStore#read} when the
    * underlying data for a tree entry is no longer available — for example, because a compactor
    * deleted the data file between the time the index was read and the time the entry was accessed.
    * <p>
    * When {@link #publish} encounters this exception it retries the iteration from the last
    * successfully processed key, up to 10 times per request. Callers should throw this instead of
    * returning {@code null} when a missing resource indicates a transient concurrent modification
   ...
   public interface PublishFunction<V, R> {
      R apply(byte[] key, V value) throws IOException;
   }
    ...
    * {@link FlowableCreate}. If the function throws
    * {@link IndexNodeOutdatedException}, the iteration retries from the last
    * successfully processed key (up to 10 retries per request).
    */
   public <R> Flowable<R> publish(PublishFunction<V, R> function) {
      return publish(function, false);
   }
   ...
   private void applyModification(Deque<PathEntry<V>> stack, Node<V> oldNode, Node<V> newNode) throws IOException {
   ...
   private ManageLengthResult<V> manageLength(Deque<PathEntry<V>> stack, Node<V> oldNode, Node<V> newNode)
      ...
      int from, to;
      if (newNode.length() < minNodeSize && !stack.isEmpty()) {
