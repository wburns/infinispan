 ...
 * <p>
 * When a transaction need to waits for older topology transaction, this class allows it check the state. If the
 * transaction does not need to wait, or all older transaction has finished or has timed out, the {@link #isReady()}
 * method returns {@code true}. Also, it allows to add listeners to be notified when it is ready.
 *
 * @author Pedro Ruivo
 * @since 8.0
 */
