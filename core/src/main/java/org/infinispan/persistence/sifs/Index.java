...
import org.infinispan.util.BPlusTree.IndexNodeOutdatedException;
...
class Index {
   ...
   static class IndexFileNodeStore implements SoftBPlusTree.NodeStore {
      ...
      ByteBuffer serializeFreeBlocks() {
         // numSizeClasses(4) + per class: blockLength(4) + listSize(4) + entries(10 each)
         int size = 4;
         for (var entry : freeBlocks.entrySet()) {
            size += 8 + entry.getValue().size() * 10;
         }
         ByteBuffer buf = ByteBuffer.allocate(size);
         buf.putInt(freeBlocks.size());
         for (var entry : freeBlocks.entrySet()) {
            List<SoftBPlusTree.NodeSpace> list = entry.getValue();
   ...
   static class Segment extends CompletableFuture<Void> implements Consumer<IndexRequest>, Action {
      ...
      volatile BPlusTree<IndexEntry> tree;
      ...
      boolean load() throws IOException {
         ...
         try (handle) {
            ...
            if (header.getInt(0) != GRACEFULLY || header.getInt(4) != segmentMax) {
               handle.truncate(0);
               tree = new BPlusTree<>(index.minNodeSize, index.maxNodeSize);
               return false;
            }
   ...
   Flowable<EntryRecord> publish(int cacheSegment, boolean loadValues) {
      ...
      try {
         ...
         BPlusTree<IndexEntry> tree = segment.tree;

         long wallClockTime = timeService.wallClockTime();
         return tree.publish((key, entry) ->
                     entry.loadRecord(dataFileProvider, wallClockTime, loadValues, false))
