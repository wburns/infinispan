...
public interface InfinispanPublisher<T> {
   ...
   <R> CompletionStage<R> compose2(Function<? super Publisher<T>, ? extends CompletionStage<R>> transformer,
    ...
    * {@link #compose(CachePublisherTransformer, CachePublisherTransformer)}  whenever possible as
    * it has a severely reduced cost of guaranteeing exactly-once semantics. The other option is to use an
   ...
   <R> Publisher<R> compose2(Function<? super Publisher<T>, ? extends Publisher<R>> publisherFunction);

   @FunctionalInterface
   interface CachePublisherTransformer<T, R> extends Function<Publisher<T>, CompletionStage<R>> {
   }
