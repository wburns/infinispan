...
public interface StoreConfigurationChildBuilder<S> extends ConfigurationChildBuilder {
    ...
    * the shared store must implement the SegmentedLoader | SegmentedWriter interfaces. Segmented stores help performance
    * for things that require viewing the entire contents of the store (eg. iteration, stream processing, state transfer,
    * mass indexer). If the store doesn't provide constant time operations for methods such as
    * {@link org.infinispan.persistence.spi.CacheLoader#load(Object)} or
