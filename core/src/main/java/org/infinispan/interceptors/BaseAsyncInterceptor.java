...
public abstract class BaseAsyncInterceptor implements AsyncInterceptor {
   ...
   public final Object asyncInvokeNext(InvocationContext ctx, VisitableCommand command,
      ...
      if (delay == null || CompletionStages.isComplete(delay)) {
    ...
    */
   public static Object stageOrSyncValue(CompletionStage<?> stage, Object syncValue) {
      if (stage != null) {
   ...
   /**
    * This method should be used instead of {@link #stageOrSyncValue(CompletionStage, Object)} when a
    ...
    * @return
    */
   public static Object stageOrSyncValue(CompletionStage<?> stage, Object syncValue, Throwable throwable) {
      if (throwable == null) {
         return stageOrSyncValue(stage, syncValue);
      }
      if (stage != null) {
         CompletableFuture<?> future = stage.toCompletableFuture();
         if (!future.isDone() || future.isCompletedExceptionally()) {
            return asyncValue(
                  stage.handle((ignore, t) -> {
                     ...
                     return null;
                  }).thenApply(ignore -> CompletableFutures.completedExceptionFuture(throwable))
            );
         }
      }
      return new SimpleAsyncInvocationStage(throwable);
   ...
   /**
    * The same as {@link #stageOrSyncValue(CompletionStage, Object)}, except that it is optimizes cases where the stage
    * is void and the sync return value is also null.
    * @param stage wait for completion of this if not null
    * @return invocation stage or null sync value
    */
   public static Object stageOrNullSyncValue(CompletionStage<Void> stage) {
      // If stage was null - meant we didn't notify or if it already completed, no reason to create a stage instance
      if (stage == null || CompletionStages.isComplete(stage)) {
         return null;
      } else {
         return asyncValue(stage);
      }
