...
public class DistributionBulkInterceptor<K, V> extends DDAsyncInterceptor {
   ...
   protected static class BackingEntrySet<K, V> extends AbstractCloseableIteratorCollection<CacheEntry<K, V>, K, V>
      ...
      public CacheStream<CacheEntry<K, V>> stream() {
         ...
         CacheStream<CacheEntry<K, V>> cacheStream = new DistributedCacheStream<>(false, null,
               !command.hasAnyFlag(FlagBitSets.SKIP_CACHE_LOAD),
               cache.getCacheConfiguration().clustering().stateTransfer().chunkSize(), registry,
      ...
      public CacheStream<CacheEntry<K, V>> parallelStream() {
         AdvancedCache<K, V> advancedCache = cache.getAdvancedCache();
         ComponentRegistry registry = advancedCache.getComponentRegistry();
         CacheStream<CacheEntry<K, V>> cacheStream = new DistributedCacheStream<>(true, null,
               !command.hasAnyFlag(FlagBitSets.SKIP_CACHE_LOAD),
               cache.getCacheConfiguration().clustering().stateTransfer().chunkSize(), registry,
               StreamMarshalling.entryToKeyFunction());
         return applyTimeOut(cacheStream, cache);
   ...
   protected static class TxBackingEntrySet<K, V> extends BackingEntrySet<K, V> {
      ...
      public CacheStream<CacheEntry<K, V>> stream() {
         ...
         CacheStream<CacheEntry<K, V>> cacheStream = new DistributedCacheStream<>(false, ctx,
               !command.hasAnyFlag(FlagBitSets.SKIP_CACHE_LOAD),
               cache.getCacheConfiguration().clustering().stateTransfer().chunkSize(), registry,
               StreamMarshalling.entryToKeyFunction());
         return applyTimeOut(cacheStream, cache);
      ...
      public CacheStream<CacheEntry<K, V>> parallelStream() {
         ...
         CacheStream<CacheEntry<K, V>> cacheStream = new DistributedCacheStream<>(true, ctx,
               !command.hasAnyFlag(FlagBitSets.SKIP_CACHE_LOAD),
               cache.getCacheConfiguration().clustering().stateTransfer().chunkSize(), registry,
               StreamMarshalling.entryToKeyFunction());
         return applyTimeOut(cacheStream, cache);
   ...
   protected static class BackingKeySet<K, V> extends AbstractCloseableIteratorCollection<K, K, V>
      ...
      public CacheStream<K> stream() {
         AdvancedCache<K, V> advancedCache = cache.getAdvancedCache();
         ComponentRegistry registry = advancedCache.getComponentRegistry();
         return new DistributedCacheStream<>(false, null, !command.hasAnyFlag(FlagBitSets.SKIP_CACHE_LOAD),
                 cache.getCacheConfiguration().clustering().stateTransfer().chunkSize(), registry, null);
      }
      ...
      public CacheStream<K> parallelStream() {
         AdvancedCache<K, V> advancedCache = cache.getAdvancedCache();
         ComponentRegistry registry = advancedCache.getComponentRegistry();
         return new DistributedCacheStream<>(true, null, !command.hasAnyFlag(FlagBitSets.SKIP_CACHE_LOAD),
                 cache.getCacheConfiguration().clustering().stateTransfer().chunkSize(), registry, null);
      }
   }
   ...
   private static class TxBackingKeySet<K, V> extends BackingKeySet<K, V> {
      ...
      public CacheStream<K> stream() {
         ...
         return new DistributedCacheStream<>(false, ctx, !command.hasAnyFlag(FlagBitSets.SKIP_CACHE_LOAD),
                 cache.getCacheConfiguration().clustering().stateTransfer().chunkSize(), registry, null);
      }
      ...
      public CacheStream<K> parallelStream() {
         ...
         return new DistributedCacheStream<>(true, ctx, !command.hasAnyFlag(FlagBitSets.SKIP_CACHE_LOAD),
                 cache.getCacheConfiguration().clustering().stateTransfer().chunkSize(), registry, null);
      }
   }
}
