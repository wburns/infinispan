 ...
 * passivated to a persistence store. Without the fix, the local node only
 * scans the in-memory data container ({@code dataContainer.forEach()}) and
 * misses passivated entries, causing the merge policy to see null instead of
 * the real value.
 */
