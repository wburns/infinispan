...
Filters optimize performance for write operations by reducing the total number of invalidation messages.

Filters:

* Reside on {brandname} Server and keep track of the entries that the client has requested.
* Use a server-side managed Cuckoo filter by default, where entries are added on read and removed on invalidation without requiring client-side recalculations.
