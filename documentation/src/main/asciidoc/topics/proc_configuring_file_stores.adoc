...
. Optionally set the `flush-mutation-count` attribute on the `index` element to tune how many buffered index updates {brandname} accumulates before flushing them to disk.
...
[discrete]
== Buffering index updates

Set the `flush-mutation-count` attribute to control how many buffered index updates {brandname} accumulates before flushing them to disk.
Increasing this value coalesces more updates for better write throughput, while a value of `1` disables buffering so that each mutation is written immediately.

.XML
[source,xml,options="nowrap",subs=attributes+,role="primary"]
----
include::xml/persistence_file_store_index_buffering.xml[]
----

.JSON
[source,json,options="nowrap",subs=attributes+,role="secondary"]
----
include::json/persistence_file_store_index_buffering.json[]
----

.YAML
[source,yaml,options="nowrap",subs=attributes+,role="secondary"]
----
include::yaml/persistence_file_store_index_buffering.yaml[]
----
