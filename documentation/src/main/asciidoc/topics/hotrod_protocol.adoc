...
* link:#hot_rod_protocol_4_0[Hot Rod Protocol 4.0 (Infinispan 14.0)]
* link:#hot_rod_protocol_4_1[Hot Rod Protocol 4.1 (Infinispan 15.1)]
...
applied to longer values. They are between 1 and 9 bytes long.
...
+0x47+ = addNearCacheListener (since 4.1) +
...
topology-aware or hash-distribution-aware clients when there has been a cluster
or view formation change. Note that it is the server that makes the decision on
whether it sends back the new topology based on the current topology id and
the one the client sent. If they are different, it will send back the new topology.
...
It is important to note that since hash headers rely on the consistent hash
...
They need to be unique per update at the key level.
| Value Length        | vInt       | If success, length of value
| Value               | byte array | If success, the requested value
|==============================================================================
...
entries need to be read from the stream. So, when it is set to 1, it means
that an entry follows, whereas when it is set to 0, it is the end of stream and
...
idle before it is evicted from the cache. If 0, no max idle time.
...
Bulk operation to remove all entries that map to a given set of keys.
...
| Header              | variable   | Request header
| Key count           | vInt       | How many keys to remove
| Key 1 Length        | vInt       | Length of key
| Key 1               | byte array | Key to remove
| Key 2 Length        | vInt       |
| Key 2               | byte array |
|... continues until key count is reached||
...
| Header              | variable   | Response header
| Response status     | 1 byte     |
+0x00+ = success, if all removals succeeded +
|==============================================================================
...
operation but remote Hot Rod clients could implement this type of
...
virtual nodes in the clusters (this number could be in the millions).
...
different virtual nodes, and to calculate each virtual node's hash code, take the range of numbers between 0 and N-1 and apply the
following logic:
...
+0x00+ = success, data follows +
| More                | 1 byte     | One byte representing whether more
keys need to be read from the stream. So, when it is set to 1, it means
that an entry follows, whereas when it is set to 0, it is the end of stream and
no more entries are left to read. For more information on BulkGet look
link:http://community.jboss.org/docs/DOC-15592[here]
| Key 1 Length        | vInt       | Length of key
...
elements since they are not in use, and even if they were in use, there are
...
for requests because either stopped, they are stopping or similar situation.
...
it means that the cache entry was not present, and hence there is no previous value
...
| Mech 1 | string | String containing the name of the SASL mech in its IANA-registered form (for example, GSSAPI, SCRAM-SHA-256)
| Mech 2 | string |
| ...etc |        |
|==============================================================================
...
| Max Idle            | vLong       | Duration that each entry can be idle before it is evicted from the cache. Only sent when time unit is not DEFAULT or INFINITE
|==============================================================================
...
For example, a client can indicate that only created events, or only expiration and removal events are of interest.
More fine-grained event interests, such as per key, can be defined using the key/value filter parameter.
...
This Hot Rod protocol version adds support for transaction operations and multimap caches.

It includes 9 new operations to manipulate multimap caches:

* request 0x67 - response 0x68 = Get a key and retrieve multiple values
* request 0x69 - response 0x6A = Get a key with metadata and retrieve multiple values
* request 0x6B - response 0x6C = Put a key/value
* request 0x6D - response 0x6E = Remove a key
* request 0x6F - response 0x70 = Remove an entry
* request 0x71 - response 0x72 = Get the cache size
* request 0x73 - response 0x74 = Contains entry
* request 0x75 - response 0x76 = Contains key
* request 0x77 - response 0x78 = Contains value

Multimap operations request and response follow the cache operations request and response formats.

It includes 3 new operations for transactions:

* Prepare, with the transaction write set (that is, modified keys), it tries to prepare and validate the transaction in the server.
...
Note that `NOT_READ` and `NON_EXISTING` cannot be set at the same time.

|Version Read
|long
|The version read. Only sent when `NOT_READ` and `NON_EXISTING` are not present.
...
+0x07+ = `DEFAULT` +
+0x08+ = `INFINITE` +
Only sent when `REMOVE_OPERATION` is not set.

| Lifespan
| vLong
| Duration which the entry is allowed to life.
Only sent when time unit is not `DEFAULT` or `INFINITE` and `REMOVE_OPERATION` is not set.

| Max Idle
| vLong
| Duration that each entry can be idle before it is evicted from the cache.
Only sent when time unit is not `DEFAULT` or `INFINITE` and `REMOVE_OPERATION` is not set.

|Value Length
|vInt
|Length of value.
Only sent if `REMOVE_OPERATION` is not set.
...
This field is not present if the response state is different from `Successful`.
|===
...
* `0x02`: The counter is not defined. +
...
Creates a counter if it does not exist.
...
NOTE: Since the `WeakCounter` does not have access to the new value, the `value` is zero.
...
These modifications of the protocol do not require any changes on the client side (as the client does not send additional operations if it does not support that; the changes are more permissive to the clients)
...
| customString | string | (Optional) If a custom MediaType is supplied (type=0x02), the custom MediaType of the key, including type and subtype. For example: _text/plain_, _application/json_.
...
Where:
 ...
 hit_count: the total number of results from the query.
 hit_count_exact: is true when the hit_count is exact. Is false when the hit count value is a lower bound.
...
If the transaction is not recoverable (`Recoverable=0`), the server rollbacks the transaction if it has
...
//| Parameter       | Description | Required
//|==============================================================================
Returns the cache names as a JSON array of strings, for example `["cache1", "cache2"]`
...
| Field Name          | Size       | Value
| Header | variable | Request header
| Bloom filter bits   | byte array   | The current client bloom filter
|==============================================================================
...
.Counter get-and-set operation
Sets a value to the counter and returns the previous value.
...
|Value
|long
|(Optional) the counter's previous value.
Only present if `Status==0x00`.
...
* `0x00`: Operation successful.
* `0x02`: The counter is not defined.
* `0x04`: The counter reached its boundary.
Only possible for `STRONG` counters.
* See the link:#hot_rod_response_header[Response Header] for error codes.

== Hot Rod Protocol 4.0

.Infinispan versions
TIP: This version of the protocol is implemented since Infinispan 14.0

To better support the new API previous return values now include metadata information.
...
Now these commands can return the version information for a prior value, which can be useful to a given API.

The following replaces the Value Length and Value entries for the operations above.
...
== Hot Rod Protocol 4.1

.Infinispan Versions
TIP: This version of the protocol is implemented since Infinispan 15.1

This version officially drops support for the previous GET_STREAM and PUT_STREAM operations added in 2.6.

Instead new operations have been added to support stream commands without having to dedicate
a socket for the entirety of the get or set operation.

* GetStreamStart for starting a remote streamed get operation
* GetStreamNext for retrieving the next chunk of the value
* GetStreamEnd for closing the remote get stream operation
* PutStreamStart for starting a remote streamed put operation
* PutStreamNext for sending the next chunko of the value
* PutStreamEnd for closing the remote put stream operation

.GetStreamStart

Starts a get stream on a remote server returning it in chunks based on the
provided batch size. Returns the first chunk in the reponse along with
metadata. If the first chunk is the entirety of the value the Complete
boolean should be true as well.
...
Integer.MAX_VALUE.
| Key                 | byte array | Byte array containing the key whose value is being requested.
| Batch Size          | vInt       | How many bytes to retrieve per request.
|==============================================================================
...
+0x00+ = success, if key retrieved +
+0x02+ = if key does not exist +
| Stream ID           | Int        | If success, Identifier for remote get stream started
| Complete            | Boolean    | Whether the retrieval is already complete
| Flag                | 1 byte     | A flag indicating whether the response
contains expiration information. The value of the flag is obtained as a
bitwise OR operation between +INFINITE_LIFESPAN (0x01)+ and
`INFINITE_MAXIDLE (0x02)`.
...
This operation is retrieve the next chunk of bytes from the remote server for
the get stream. If complete is true then this is the last chunk.
...
| Header              | variable   | Request header
| Stream ID           | Int        | Stream identifier retrieved from GetStreamStart.
|==============================================================================
...
+0x02+ = if key does not exist +
| Stream ID           | Int        | If success, Identifier for remote get stream started
| Complete            | Boolean    | If success, whether all bytes have been retrieved
| Value Length        | vInt       | If success, length of value
| Value               | byte array | If success, the requested value
|==============================================================================
...
This operation is used to close a get stream that was opened on a remote server.
...
| Header              | variable   | Response header
| Response status     | 1 byte     |
+0x00+ = success, if closed +
+0x02+ = if ID does not exist +
|==============================================================================

.PutStreamStart

Opens a put stream on a remote server where it will then accept chunks of data.
This is useful to not have to have a client hold onto the entire contents
of a written value at any one time.
...
| Field Name          | Size       | Value
| Header    | variable | Response header
| Stream ID | Int      | Identifier for remote get stream started
|==============================================================================

.PutStreamNext

This operation is used to push a chunk of the value bytes to the remote server.
When complete is true the remote server will perform the operation based
upon the version supplied in the PutStreamStart operation.
...
| Header              | variable   | Request header
| Stream ID           | Int        | Stream ID retrieved from Start
| Complete            | Boolean    | If this is the last chunk for the value
| Chunk length        | vInt       | Length of the chunk for the values
| Value Chunk         | byte array | Byte array consisting of the chunk of data
|==============================================================================
...
This operation is used to close a put stream that was opened on a remote server.
...
.Add Near Cache Listener

Near caches can now utilize a server-side managed Cuckoo filter instead of the legacy Bloom filter.
With this model, keys are added on get requests and removed on cache updates, eliminating the need
for periodic client-side filter calculations and updates.
...
| Header              | variable   | Request header
| Listener ID         | byte array | Listener identifier
| Near cache size     | vInt       | Defines the maximum entries of the near cache, used to size the server-side filter.
|==============================================================================
