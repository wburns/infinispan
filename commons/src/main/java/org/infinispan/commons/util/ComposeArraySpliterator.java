 ...
 * @since 9.3
 */
public class ComposeArraySpliterator<E> implements Spliterator<E> {
   ...
   public ComposeArraySpliterator(IntFunction<Collection<E>> toCollection, int length, int additionalCharacteristics) {
      this(toCollection, length, 0, length, additionalCharacteristics);
   }

   private ComposeArraySpliterator(IntFunction<Collection<E>> toCollection, int length, int index, int fence, int characteristics) {
   ...
   public Spliterator<E> trySplit() {
      ...
      return (lo >= mid)
            ? null
            : new ComposeArraySpliterator<>(toCollection, length, lo, index = mid, characteristics);
   }
