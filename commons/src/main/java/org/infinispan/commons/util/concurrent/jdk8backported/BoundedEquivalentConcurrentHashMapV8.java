...
import org.infinispan.commons.util.concurrent.jdk8backported.CountedCompleter;
      ...
      public void onEntryRemove(Node<K, V> e) {
         ...
         synchronized (eviction) {
            if (eviction.queueNode != null) {
               // Need to set to null before unlinking
               eviction.queueNode.item = null;
               // this doesn't get unlinked if it was a tail of head here - 
      ...
      public Collection<Node<K, V>> findIfEntriesNeedEvicting() {
         ...
         if (extra > 0) {
            ...
            for (long j = 0; j < extra; ++j) {
               ...
               // TODO: should we do the increment size eviction outside of for loop ?
               if (removed) {
...
//      final AtomicLong evictingCount = new AtomicLong();
      ...
      boolean incrementSizeAndEvictingIfNecessary() {
         ...
         while (!replaced) {
            ...
            lirsSize = new SizeAndEvicting(newSize, newEvicting);
            replaced = currentSize.compareAndSet(existingSize, lirsSize);
         }
         return evict;
      }
      ...
      /**
       * Evicts a HIR entry
       * @return whether or not the eviction was stopped due to an entry being removed instead
       */
      @SuppressWarnings("unchecked")
      boolean evictHIREntry() {
            ...
            synchronized (removedHIR) {
               ...
               case HIR_RESIDENT:
                  ...
                  if (removedHIR.stackNode != null) {
                     ...
                     Collection<LIRSNode<K, V>> oldNodesToEvict = nodesToEvictTL.get();
                     if (oldNodesToEvict == null) {
                        oldNodesToEvict = Collections.singleton(removedHIR);
                        nodesToEvictTL.set(oldNodesToEvict);
                     } else {
                        oldNodesToEvict.add(removedHIR);
                     }
                  } else {
                  ...
                  break;
               case REMOVED:
                  // If the element was removed we assume we have the space for it now
                  evicted = true;
                  wasRemoval = true;
                  break;
               case LIR_RESIDENT:
      ...
      public void onEntryMiss(Node<K, V> e, V value) {
         ...
         // Only increment the size if this wasn't created yet
         boolean toEvict = !skipIncrement && incrementSizeAndEvictingIfNecessary();
         if (toEvict) {
            boolean removalEviction = evictHIREntry();
            if (removalEviction) {
               // If the entry was removed that means we didn't actually evict, so 
               // we just decrement eviction and not size
               incrementSizeEviction(currentSize, 0, -1);
            } else {
               // Lastly we reduce the size since we removed a resident
               incrementSizeEviction(currentSize, -1, -1);
            }
         } else if (evictHIR) {
            evictHIREntry();
         }
         ...
         while (!pruned) {
            ...
            if (LIRDetails == null) {
               ...
               synchronized (removedLIR) {
                  ...
                  // If the node was removed concurrently removed we don't need to prune
                  // anything because they freed a spot
      ...
      Object[] pruneIncludingLIR() {
         ...
         while (true) {
            ...
            synchronized (removedLIR) {
               ...
               case EVICTING:
               case EVICTED:
                  // We ignore this value if it it was evicted elsewhere as it is no
                  // longer LIRS
                  break;
               }
            }
      ...
      public void onEntryHitWrite(Node<K, V> e, V value) {
         ...
         synchronized (lirsNode) {
            ...
            case HIR_NONRESIDENT:
               ...
               break;
            case EVICTED:
               // TODO: we can't reliably do a put here without having a point where the object
               // wasn't seen is that okay?
               break;
            case EVICTING:
               // In the case of eviction we add the node back as if it was a HIR_RESIDENT
               // except we also have to evict an old HIR to make room
      ...
      @Override
      public Collection<Node<K, V>> findIfEntriesNeedEvicting() {
         Collection<LIRSNode<K, V>> evicting = nodesToEvictTL.get();
         if (evicting != null) {
            nodesToEvictTL.remove();
            Collection<Node<K, V>> removedNodes = new ArrayList<>(evicting.size());
            for (LIRSNode<K, V> evict : evicting) {
               ...
               if (evict.state == Recency.EVICTING || 
                     evict.state == Recency.HIR_NONRESIDENT /* TODO find way to double check this easily ? && evict.attachedNode.val != null*/) {
   ...
   public interface BiAction<A,B> extends KeyValueAction<A, B> { void apply(A a, B b); }
   ...
   static class Node<K,V> implements Map.Entry<K,V> {
      ...
      // TODO: this can throw a NPE since value can be null
...
//      logMessage("Found size to be: %s", currentSize);
      notifyEvictionListener(evictionPolicy.findIfEntriesNeedEvicting());
      return null;
   }
...
//         assert checkInvariants(root);
   ...
   public void forEach(long parallelismThreshold,
         KeyValueAction<? super K,? super V> action) {
      if (action == null) throw new NullPointerException();
   ...
   static final class EntrySetView<K,V> extends CollectionView<K,V,Map.Entry<K,V>>
      ...
      public final int hashCode() {
         ...
         if ((t = map.table) != null) {
            Traverser<K,V> it = new Traverser<K,V>(t, t.length, 0, t.length);
            for (Node<K,V> p; (p = it.advance()) != null; ) {
               // TODO: need to worry about null value here
               h += p.hashCode();
            }
         }
         return h;
      ...
      public void forEach(Action<? super Map.Entry<K,V>> action) {
         ...
         if ((t = map.table) != null) {
            ...
            for (Node<K,V> p; (p = it.advance()) != null; ) {
               V val = p.val;
               if (val != null) {
                  action.apply(new MapEntry<K,V>(p.key, val, map));
               }
            }
         }
      }
   ...
   static final class ForEachMappingTask<K,V>
         extends BulkTask<K,V,Void> {
      final KeyValueAction<? super K, ? super V> action;
      ForEachMappingTask
            (BulkTask<K,V,?> p, int b, int i, int f, Node<K,V>[] t,
                  KeyValueAction<? super K,? super V> action) {
         super(p, b, i, f, t);
