...
public class CommonHandler {
   ...
   public CommonHandler(HotRodServer server, NettyTransport transport) {
      this.server = server;
      this.transport = transport;
   }

   public boolean handle(ChannelHandlerContext ctx, CacheDecodeContext msg) throws ExecutionException, InterruptedException {
      ...
      switch (h.op()) {
         ...
         case IterationStartRequest:
            Tuple4<Option<byte[]>, Option<Tuple2<String, List<byte[]>>>, Integer, Boolean> iterationStart =
   ...
   public IllegalArgumentException operationNotSupported(HotRodOperation op) {
      return new IllegalArgumentException("Unsupported operation invoked: " + op);
   }
}
