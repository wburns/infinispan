...
public class Decoder2xJava {
   ...
   static final long EXPIRATION_NONE = -1;
   static final long EXPIRATION_DEFALUT = -2;

   static final ExpirationParam DEFAULT_EXPIRATON = new ExpirationParam(-1, TimeUnitValue.SECONDS());
    ...
    * @param header
    * @param buffer
    * @return null if not enough bytes othwerwise an optional value
    */
   ...
   private static RequestParameters readParameters(ByteBuf buffer, HotRodHeader header, boolean readExpiration,
      ...
      if (readExpiration) {
         ...
         param1 = param2 = DEFAULT_EXPIRATON;
      }
      if (readVersion) {
   ...
   private static ExpirationParam readExpirationParam(boolean pre22Version, boolean useDefault, ByteBuf buffer,
      ...
      if (pre22Version) {
         ...
         if (duration == Integer.MIN_VALUE) {
            ...
            duration = useDefault ? (int) EXPIRATION_DEFALUT : (int) EXPIRATION_NONE;
         }
         ...
         switch (timeUnit) {
            // Default time unit
            case 0x07:
               return new ExpirationParam(EXPIRATION_DEFALUT, timeUnit);
            // Infinite time unit
            case 0x08:
