...
import io.netty.util.concurrent.{EventExecutorGroup, DefaultEventExecutorGroup, DefaultThreadFactory}
...
class HotRodChannelInitializer(val server: HotRodServer, transport: => NettyTransport,
   ...
   override def initChannel(ch: Channel): Unit = {
      ...
      ch.pipeline.addLast(executionGroup, "logging", new HotRodLoggingHandler)
