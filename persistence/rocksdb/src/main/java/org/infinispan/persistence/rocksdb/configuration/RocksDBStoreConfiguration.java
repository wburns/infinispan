...
public class RocksDBStoreConfiguration extends AbstractNonSharedSegmentedConfiguration<RocksDBStoreConfiguration> {
   ...
   public RocksDBStoreConfiguration newConfigurationFrom(int segment) {
      ...
      set.attribute(LOCATION).set(location + "-" + segment);
      String expiredLocation = set.attribute(EXPIRED_LOCATION).get();
      set.attribute(EXPIRED_LOCATION).set(expiredLocation + "-" + segment);
