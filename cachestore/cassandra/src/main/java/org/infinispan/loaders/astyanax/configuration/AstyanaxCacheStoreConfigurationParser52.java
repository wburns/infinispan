 ...
 * @since 5.2
 */
public class AstyanaxCacheStoreConfigurationParser52 implements ConfigurationParser<ConfigurationBuilderHolder> {
   ...
   public AstyanaxCacheStoreConfigurationParser52() {
   }
   ...
   private void parseCassandraStore(final XMLExtendedStreamReader reader, LoadersConfigurationBuilder loadersBuilder,
      ...
      AstyanaxCacheStoreConfigurationBuilder builder = new AstyanaxCacheStoreConfigurationBuilder(loadersBuilder);
   ...
   private void parseServers(XMLExtendedStreamReader reader, AstyanaxCacheStoreConfigurationChildBuilder builder)
         throws XMLStreamException {
      while (reader.hasNext() && (reader.nextTag() != XMLStreamConstants.END_ELEMENT)) {
         Element element = Element.forName(reader.getLocalName());
         switch (element) {
   ...
   private void parseServer(XMLExtendedStreamReader reader, AstyanaxServerConfigurationBuilder builder)
         throws XMLStreamException {
   ...
   private void parseCassandraStoreAttributes(XMLExtendedStreamReader reader,
         AstyanaxCacheStoreConfigurationBuilder builder) throws XMLStreamException {
      for (int i = 0; i < reader.getAttributeCount(); i++) {
         ParseUtils.requireNoNamespaceAttribute(reader, i);
         String value = replaceProperties(reader.getAttributeValue(i));
         Attribute attribute = Attribute.forName(reader.getAttributeLocalName(i));
